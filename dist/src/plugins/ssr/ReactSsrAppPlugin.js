...
exports.ReactSsrAppPlugin = void 0;
const os_core_ts_1 = require("os-core-ts");
const path_1 = __importDefault(require("path"));
const compileConfig = require(path_1.default.resolve(process.cwd(), 'reactSsrCompilerConfig.js'));
const build = require(path_1.default.join(process.cwd(), ...compileConfig.serverBuildDirectory.split('/'), 'index.js'));
const argEndpointsHandler = new os_core_ts_1.ArgEndpointsHandler();
class ReactSsrAppPlugin {
    constructor() {
        this.ssrGlobalDataController = null;
        this.notFoundHandler = null;
    }
    register(app) {
        app.routerBuilder.registerRouteHandler('ReactSsr', (router, endpoint, controller) => {
            router.get(endpoint.path, this.getSSRControllerHandler(endpoint, controller));
        });
    ...
    useGlobalDataController(controller) {
        this.ssrGlobalDataController = new controller();
        return this;
    }
    getSSRControllerHandler(endpoint, controller) {
        return async (req, res) => {
            const appData = await this.handleAppData(endpoint, controller, req, res, req.headers.xinitialpreload !== 'false');
            ...
            try {
                const result = build.entry.module.default(this.createAppContext(build, req, res, appData));
                if (appData.pageData.status !== 200) {
                    res.status(appData.pageData.status);
                }
                res.send(result);
                ...
                os_core_ts_1.appLogger.error('error build ssr page', error);
                res.status(500).send(this.buildHtmlErrorPage(error));
            }
        };
    }
    async handleAppData(endpoint, controller, req, res, initialPreload) {
        const pageData = await this.handlePageData(endpoint, controller, req, res);
        if (!initialPreload) {
            res._body = {
                pageData,
    ...
    async handlePageData(endpoint, controller, req, res) {
        try {
            ...
            const errorResult = os_core_ts_1.AppErrorHelper.buildErrorResultFromError(error);
            return {
                data: null,
                error: true,
    ...
    async handleGlobalData(req, res) {
        try {
            const data = await argEndpointsHandler.getDataByControllerMethod(this.ssrGlobalDataController, 'loadGlobalData', req, res);
            return {
                data,
                errors: [],
                error: false,
    ...
    createAppContext(build, req, res, appData) {
        const routeModules = {};
...
exports.ReactSsrAppPlugin = ReactSsrAppPlugin;
//# sourceMappingURL=ReactSsrAppPlugin.js.map